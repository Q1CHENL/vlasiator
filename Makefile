...
COMPFLAGS += -DDATATYPE_CONVERSION_OPTIMIZATION
...
ifeq ($(USE_CUDA),1)
	...
	GPU_CUDA_ARCH_FLAGS += -arch=sm_90
	# GPU_CUDA_ARCH_FLAGS += -rdc=true
	# COMPFLAGS += -DGPU_ANALYSIS_BUILD
	# GPU_CUDA_ARCH_FLAGS += -DGPU_ANALYSIS_BUILD
endif
ifeq ($(USE_HIP),1)
	USE_GPU=1
	LIBS += ${LIB_HIP} -lhiprt
...
help:
	...
	@echo 'make cuda_objs           build CUDA object files (when USE_CUDA=1)'
...
clean: data
	@echo "[CLEAN]"
	$(SILENT)rm -rf *.o *.d *.cubin arch/*.cubin vlasovsolver/*.cubin *~ */*~ */*/*~ ${EXE} particle_post_pusher check_projects_compil_logs/ check_projects_cfg_logs/ particles/*.o
cleantools:
...
spatial_cell.o: spatial_cell_gpu.cpp
	@echo [CC] $<
	$(SILENT)$(CMP) $(CXXFLAGS) ${MATHFLAGS} $(FLAGS) $(GPU_CUDA_ARCH_FLAGS) -c spatial_cell_gpu.cpp -o spatial_cell.o $(INC_BOOST) ${INC_DCCRG} ${INC_EIGEN} ${INC_ZOLTAN} ${INC_VECTORCLASS} ${INC_FSGRID}
gpu_%.o: vlasovsolver/gpu_%.cpp vlasovsolver/vec.h
	@echo [CC] $<
	$(SILENT)${CMP} ${CXXFLAGS} ${MATHFLAGS} ${FLAGS} $(GPU_CUDA_ARCH_FLAGS) -c $< -I$(CURDIR) ${INC_BOOST} ${INC_EIGEN} ${INC_DCCRG} ${INC_FSGRID} ${INC_ZOLTAN} ${INC_PROFILE} ${INC_VECTORCLASS} ${INC_EIGEN} ${INC_VLSV} ${INC_MPI}
gpu_%.o: arch/gpu_%.cpp
	@echo [CC] $<
	$(SILENT)${CMP} ${CXXFLAGS} ${MATHFLAGS} ${FLAGS} $(GPU_CUDA_ARCH_FLAGS) -c $< -I$(CURDIR) ${INC_BOOST} ${INC_EIGEN} ${INC_DCCRG} ${INC_FSGRID} ${INC_ZOLTAN} ${INC_PROFILE} ${INC_VECTORCLASS} ${INC_EIGEN} ${INC_VLSV} ${INC_MPI}
else
...
# Build per-file cubins for GPU translation units (analyze modules individually)
ifeq ($(USE_GPU),1)

# CUDA TU sources that contain kernels
GPU_CUBINS = \
	arch/gpu_base.cubin \
	vlasovsolver/gpu_acc_map.cubin \
	vlasovsolver/gpu_acc_semilag.cubin \
	vlasovsolver/gpu_acc_sort_blocks.cubin \
	vlasovsolver/gpu_trans_map_amr.cubin \
	vlasovsolver/gpu_dt.cubin \
	vlasovsolver/gpu_moments.cubin \
	spatial_cell_gpu.cubin

# Generic cubin build rules (nvcc treats .cpp as CUDA via -x cu in CXXFLAGS)
%.cubin: %.cpp
	@echo [CUBIN] $@
	$(SILENT)$(CMP) -I$(CURDIR) $(CXXFLAGS) -cubin -o $@ $<

cubins: $(GPU_CUBINS)
	@echo "[DONE] Built CUDA module cubins"

# CUDA object files (build .o without linking)
GPU_OBJS = \
	gpu_base.o \
	gpu_acc_map.o \
	gpu_acc_semilag.o \
	gpu_acc_sort_blocks.o \
	gpu_trans_map_amr.o \
	gpu_dt.o \
	gpu_moments.o \
	spatial_cell.o
...
cuda_objs: $(GPU_OBJS)
	@echo "[DONE] Built CUDA object files"
